...
const walk = (dir) => {
  for (const entry of readdirSync(dir, { withFileTypes: true })) {
    ...
    // *.bundle.json are generated by bundle-dishes.mjs from the per-category
    // folders below — validating them too would flag every id as a duplicate.
    else if (entry.name.endsWith('.json') && !entry.name.endsWith('.bundle.json')) validateFile(p);
  }
};
